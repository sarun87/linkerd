...
import com.timgroup.istio.NoOpIstioClient
import org.scalatest._

class IstioStatsReceiverTest extends FunSuite {

  test("creates a stats receiver") {
    val stats = new IstioStatsReceiver(new NoOpIstioClient, 1.0d)
    assert(stats.isInstanceOf[IstioStatsReceiver])
  }

  test("stops IstioClient on close") {
    val istioClient = new MockIstioClient
    val stats = new IstioStatsReceiver(istioClient, 1.0d)
    assert(!istioClient.stopped)
    stats.close()
    assert(istioClient.stopped)
  }
  ...
  test("metric names are correctly rewritten") {
    ...
    val newNames = names.map { IstioStatsReceiver.mkName(_) }
    val expected = Seq(
