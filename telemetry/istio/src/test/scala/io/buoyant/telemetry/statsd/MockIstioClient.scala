...
import com.timgroup.istio.IstioClient

class MockIstioClient extends IstioClient {
  var lastName = ""
  var lastValue = ""
  var stopped = false
  ...
  // we can't simply extend NoOpIstioClient because it's final
  ...
  def recordEvent(x$1: com.timgroup.istio.Event, x$2: String*): Unit = ???
  ...
  def recordServiceCheckRun(x$1: com.timgroup.istio.ServiceCheck): Unit = ???
  def recordSetValue(x$1: String, x$2: String, x$3: String*): Unit = ???
  def serviceCheck(x$1: com.timgroup.istio.ServiceCheck): Unit = ???
